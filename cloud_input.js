var CloudInputObj = function () {
	...
	this.punct = 0;
	this.server_base_url = "http://127.0.0.1:8080/";
	this.shape = 0;
	this.static_prefix = "";
	this.turn_on = false;
	this.type_on = false;
...
CloudInputObj.prototype.addCSSandHTML = function () {
	...
	$("<div id='cloud_input_status'>\
		<table>\
		<tr>\
		<td><img src='"+this.static_prefix+"img/pin50.png'></td>\
		<td><img src='"+this.static_prefix+"img/banjiao50.png'></td>\
		<td><img src='"+this.static_prefix+"img/quan50.png'></td>\
		</tr>\
		</table>\
		</div>").appendTo("body");
	$("#cloud_input_status").hide();
		
	$("<link rel='stylesheet' type='text/css' href='"+this.static_prefix+"cloud_input.css'>").appendTo("head");
};
...
CloudInputObj.prototype.bindEvents = function () {
	...
	$(".cloud_input_drct").eq(0).mousedown(function(e){
		e.preventDefault();
		CloudInput.candidatePageUp.call(CloudInput);
	});
	
	$(".cloud_input_drct").eq(1).mousedown(function(e){
		e.preventDefault();
	...
	$("#cloud_input_status img").eq(0).click(function(e){
		e.preventDefault();
		CloudInput.switchMethod.call(CloudInput, e.target);
	});
	
	$("#cloud_input_status img").eq(1).click(function(e){
		e.preventDefault();
		CloudInput.switchShape.call(CloudInput, e.target);
	});
	
	$("#cloud_input_status img").eq(2).click(function(e){
		e.preventDefault();
	...
	$("input, textarea").keypress(function(e) {
		...
		if(CloudInput.chi_mode){
			...
			if(e.which >= 97 && e.which <=122) {
				...
				CloudInput.setPreeditText.call(CloudInput, CloudInput.getPreeditText()+String.fromCharCode(e.which));
				CloudInput.fetchCandidateAsync.call(CloudInput);
			}else if(e.which == 13) {
				/*Enter键时候直接上屏preedit*/
...
CloudInputObj.prototype.candidateNext = function () {
	this.cand_index = (this.cand_index + 1 <= this.cands.length) ? this.cand_index + 1 : this.cands.length - 1;
	if (this.cand_index % 5 == 0) {
		this.cand_page = ((this.cand_page + 1) < (this.cands.length / 5)) ? (this.cand_page + 1) : Math.floor(this.cands.length / 5);
	}
	this.showCandidates(true);
};
...
CloudInputObj.prototype.candidatePageDown = function () {
	this.cand_index = ((this.cand_page + 1) < (this.cands.length / 5)) ? this.cand_index + 5 : this.cand_index;
	this.cand_page = ((this.cand_page + 1) < (this.cands.length / 5)) ? (this.cand_page + 1) : Math.floor(this.cands.length / 5);
	this.showCandidates(true);
};
...
CloudInputObj.prototype.commitText = function () {
	...
	this.new_in.val(this.new_in.val()+str);
	this.setPreeditText("");
};
...
CloudInputObj.prototype.fetchCandidateAsync = function () {
	...
	$.ajax({
		...
		dataType: data_type,
		data: {
			text: this.getPreeditText()
		},
		jsonp: "cb",
		timeout: 5000,
		error:function() {
			CloudInput.showCandidates.call(CloudInput, false);
		},
...
CloudInputObj.prototype.getServerURL = function () {
	switch (this.method) {
	case 1:
		return this.server_base_url + "pinyin";
	case 2:
		return this.server_base_url + "cangjie";
	}
};
...
CloudInputObj.prototype.highBack = function (){
	$('.cloud_input_cnd').css({
		...
		"font-size":"100%",
		 "opacity":"1.0"});
	$('.cloud_input_cnd').eq(this.cand_index % 5).css({
...
CloudInputObj.prototype.init = function(static_prefix) {
...
CloudInputObj.prototype.setServerURL = function (url) {
...
CloudInputObj.prototype.switchMethod = function (t) {
	this.method = (this.method + 1) % 3;
	switch (this.method) {
	case 0:
		this.chi_mode = false;
		t.src = this.static_prefix + "img/en50.png";
		break;
	case 1:
		this.chi_mode = true;
		t.src = this.static_prefix + "img/pin50.png";
		break;
	case 2:
		this.chi_mode = true;
		t.src = this.static_prefix + "img/cang50.png";
		break;
	}
};
...
CloudInputObj.prototype.switchShape = function (t) {
	...
	switch (this.shape) {
	case 0:
		t.src = this.static_prefix + "img/banjiao50.png";
		break;
	case 1:
		t.src = this.static_prefix + "img/quanjiao50.png";
		break;
	}
};
...
CloudInputObj.prototype.switchPunct = function (t) {
	...
	switch (this.punct) {
	case 0:
		t.src = this.static_prefix + "img/ban50.png";
		break;
	case 1:
		t.src = this.static_prefix + "img/quan50.png";
		break;
	}
};
